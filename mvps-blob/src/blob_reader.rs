...
use crate::{consts::BLOB_MAGIC, interfaces::RemoteBlob};
...
impl BlobReader {
  pub async fn open(backend: Rc<dyn RemoteBlob>) -> anyhow::Result<Self> {
    ...
    if header.version != 1 {
      anyhow::bail!("unsupported version in header");
    }
